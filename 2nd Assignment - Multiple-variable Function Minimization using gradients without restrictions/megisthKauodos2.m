...
% Gradient Descent with Optimal Step Size (Exact Line Search)
function [x_vals, y_vals, f_values] = gradient_descent_optimal(f, grad_f, x0, max_iter, epsilon)
...
function [x_vals, y_vals, f_values] = gradient_descent_armijo(f, grad_f, x0, max_iter, epsilon, alpha, beta, armijo_initial)
    x = x0;
    x_vals = x(1);
    y_vals = x(2);
    f_values = [];
...
gamma_fixed = 3.5;  % Fixed step size
armijo_initial = 6;
alpha = 0.2;  % Armijo condition parameter
beta = 0.01;  % Armijo step size reduction factor

% Run the three methods
[x_vals_fixed, y_vals_fixed, f_values_fixed] = gradient_descent_fixed(f, grad_f, x0, max_iter, epsilon, gamma_fixed);
[x_vals_optimal, y_vals_optimal, f_values_optimal] = gradient_descent_optimal(f, grad_f, x0, max_iter, epsilon);
[x_vals_armijo, y_vals_armijo, f_values_armijo] = gradient_descent_armijo(f, grad_f, x0, max_iter, epsilon, alpha, beta, armijo_initial);
...
title('Gradient Descent Trajectories for starting point ( ' + string(strjoin(string(x0)))  + ' )'  );
xlabel('x');
ylabel('y');
zlabel('f(x, y)');
grid on;
